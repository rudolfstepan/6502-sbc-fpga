...
entity tb_sbc_t65_boot is
end entity;

architecture sim of tb_sbc_t65_boot is
...
begin
  ...
  dut : entity work.sbc_t65_top
    generic map (
      ROM_INIT_FILE => "sim/hex/rom_t65_store.hex"
    )
    port map (
      clk          => clk,
      reset_n      => reset_n,
  ...
  begin
    ...
    for i in 0 to 512 loop
      ...
      if dbg_cpu_we = '1' and dbg_cpu_addr = x"0002" then
        assert dbg_cpu_data = x"42"
          report "T65 system write data mismatch"
          severity failure;

        report "tb_sbc_t65_boot passed";
        stop;
      end if;
    end loop;

    assert false
      report "T65 system did not execute STA $0002"
      severity failure;
  end process;
end architecture;
