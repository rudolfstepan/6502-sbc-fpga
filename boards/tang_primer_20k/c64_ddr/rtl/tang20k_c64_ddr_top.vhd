...
-- 135 MHz TMDS + 27 MHz pixel), the PT8211 audio DAC, and the local DDR3
-- bridge. The machine itself is the c64_ddr-local c64_core_ddr fork.
--
...
entity tang20k_c64_ddr_top is
  port (
    ...
    -- DDR3 SDRAM pins: the c64_ddr-local core uses the on-board DDR3 as its
    -- 64K main RAM through ddr3_byte_bridge.
...
architecture rtl of tang20k_c64_ddr_top is
  ...
  constant C64_RESET_HOLD_CYCLES  : integer := 270_000; -- 10 ms after DDR/boot
  constant C64_AUTO_RESET_DELAY_CYCLES : integer := 6_750_000; -- 250 ms running
  constant C64_AUTO_RESET_PULSE_CYCLES : integer := 1_350_000; -- 50 ms reset pulse
  ...
  -- ---- DDR3 main RAM backend for the local c64_ddr core fork ----
  signal c64_reset_n        : std_logic;
  signal c64_reset_reg      : std_logic := '0';
  signal c64_reset_cnt      : integer range 0 to C64_RESET_HOLD_CYCLES := C64_RESET_HOLD_CYCLES;
  signal auto_board_reset_n : std_logic := '1';
  signal c64_auto_delay_cnt : integer range 0 to C64_AUTO_RESET_DELAY_CYCLES := C64_AUTO_RESET_DELAY_CYCLES;
  signal c64_auto_pulse_cnt : integer range 0 to C64_AUTO_RESET_PULSE_CYCLES := C64_AUTO_RESET_PULSE_CYCLES;
  type c64_reset_state_t is (CR_WAIT_RAM, CR_INITIAL_HOLD, CR_RUN_DELAY, CR_AUTO_PULSE, CR_DONE);
  signal c64_reset_state : c64_reset_state_t := CR_WAIT_RAM;
  signal ddr_memory_clk     : std_logic;
  signal ddr_pll_lock       : std_logic;
  signal ddr_clk_x1         : std_logic;
  ...
  constant DDR_RST_HOLD     : integer := 1023;
  constant DDR_CAL_WAIT     : integer := 540_000;
  type ddr_rst_state_t is (DR_ASSERT, DR_WAIT_CAL);
  signal ddr_rst_state      : ddr_rst_state_t := DR_ASSERT;
  signal ddr_rst_n          : std_logic := '0';
  ...
  signal ram_test_fail_addr : std_logic_vector(15 downto 0);
  signal ram_test_expected  : std_logic_vector(7 downto 0);
  signal ram_test_actual    : std_logic_vector(7 downto 0);
...
begin
  ...
  reset_n <= base_reset_n and auto_board_reset_n;
  c64_reset_n <= c64_reset_reg and reset_n;

  led(0) <= not ddr_pll_lock       when ram_ready = '0' else
            '0' when led_hold_read     /= 0 else '1';
  led(1) <= not ddr_calib_complete when ram_ready = '0' else
            '0' when led_hold_write    /= 0 else '1';
  led(2) <= not ram_test_active    when ram_ready = '0' else
            '0' when led_hold_sd_read  /= 0 else '1';
  led(3) <= '0' when ram_test_error = '1' else
            '0' when led_hold_sd_write /= 0 else '1';
  ...
  begin
    if rising_edge(clk_pix) then
      if base_reset_n = '0' then
        c64_reset_reg <= '0';
        auto_board_reset_n <= '1';
        c64_reset_state <= CR_WAIT_RAM;
        c64_reset_cnt <= C64_RESET_HOLD_CYCLES;
        c64_auto_delay_cnt <= C64_AUTO_RESET_DELAY_CYCLES;
        c64_auto_pulse_cnt <= C64_AUTO_RESET_PULSE_CYCLES;
      elsif boot_owns = '1' and c64_reset_state /= CR_AUTO_PULSE
            and c64_reset_state /= CR_DONE then
        c64_reset_reg <= '1';
        auto_board_reset_n <= '1';
        c64_reset_state <= CR_WAIT_RAM;
        c64_reset_cnt <= C64_RESET_HOLD_CYCLES;
        ...
        case c64_reset_state is
          when CR_WAIT_RAM =>
            ...
            c64_auto_pulse_cnt <= C64_AUTO_RESET_PULSE_CYCLES;
            if ram_ready = '1' then
              c64_reset_state <= CR_INITIAL_HOLD;
            end if;

          when CR_INITIAL_HOLD =>
            c64_reset_reg <= '0';
            auto_board_reset_n <= '1';
            if ram_ready = '0' then
              c64_reset_state <= CR_WAIT_RAM;
            elsif c64_reset_cnt > 0 then
              c64_reset_cnt <= c64_reset_cnt - 1;
            else
              c64_reset_reg <= '1';
              c64_reset_state <= CR_RUN_DELAY;
            end if;
          ...
          when CR_RUN_DELAY =>
            ...
            if ram_ready = '0' then
              c64_reset_reg <= '0';
              c64_reset_state <= CR_WAIT_RAM;
            elsif c64_auto_delay_cnt > 0 then
  ...
  -- DDR3 controller + byte bridge. The c64_ddr core runs on clk_pix, so the
  -- bridge's sys side is clocked there; the Gowin app side uses clk_x1.
  ddr_cs     <= '0';
  ...
  ddr_bridge_i : entity work.ddr3_byte_bridge
    generic map (ADDR_BITS => 16, MASK_BIT_MASKS => true)
    port map (
      clk_sys   => clk_pix,
      rst_sys_n => pll_lock,
      req       => c64_ram_req,
  ...
  c64_i : entity work.c64_core_ddr
    generic map (
      -- true  = vic_ii_xl: cycle-based 6569 (badlines, border flip-flops,
      --         YSCROLL/RSEL, mid-line register effects, sprite Y-crunch,
      --         collision IRQs). In this DDR fork it reads the local 16K VIC
      --         shadow instead of touching DDR directly.
      -- false = simple line-buffer VIC for early bring-up.
      VIC_XL => true,
      IEC_BUS_MODEL => true,
    ...
    port map (
      ...
      ext_ram_req       => c64_ram_req,
      ext_ram_we        => c64_ram_we,
      ext_ram_addr      => c64_ram_addr,
      ext_ram_din       => c64_ram_din,
      ext_ram_dout      => c64_ram_dout,
      ext_ram_ack       => c64_ram_ack,
      ext_ram_ready     => ram_ready
    );
