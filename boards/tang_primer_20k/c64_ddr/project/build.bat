@echo off
rem ============================================================================
rem  Native C64 DDR fork - deterministic Gowin build (Tang Primer 20K).
rem
...
rem      build.bat
rem
rem  Output: impl\pnr\tang_c64_ddr.fs   (flash that).
rem ============================================================================
...
if exist "%DEVCFG%" copy /y "%DEVCFG%" "%TEMP%\c64_ddr_device.cfg.bak" >nul
...
echo.
echo Building native C64 DDR-fork bitstream ...
"%GWSH%" build.tcl
set "RC=%ERRORLEVEL%"

rem --- restore the versioned device.cfg for the next build -------------------
if exist "%TEMP%\c64_ddr_device.cfg.bak" (
    copy /y "%TEMP%\c64_ddr_device.cfg.bak" "%DEVCFG%" >nul
    del /q "%TEMP%\c64_ddr_device.cfg.bak" >nul
)
...
if exist "impl\pnr\tang_c64_ddr.fs" (
    echo ====================================================================
    echo  BUILD OK  -^>  %CD%\impl\pnr\tang_c64_ddr.fs
    echo  Flash this with the Gowin Programmer or openFPGALoader.
    echo ====================================================================
) else (
    echo  gw_sh reported success but impl\pnr\tang_c64_ddr.fs was not produced.
    exit /b 1
)
endlocal
