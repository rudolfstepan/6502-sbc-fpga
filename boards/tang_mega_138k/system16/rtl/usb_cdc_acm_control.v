...
module usb_cdc_acm_control (
  ...
  // SET_LINE_CODING arrives as seven separate bytes.  Keep them in shadow
  // registers and publish the complete format atomically with the final byte
  // so the CDC mailbox never observes a transaction-intermediate setting.
  reg [31:0] baud_shadow;
  reg [7:0] stop_bits_shadow;
  reg [7:0] parity_shadow;
    ...
    if (reset) begin
      ...
      if (setup) begin
        ...
        if (rx_valid) begin
          ...
          7: begin
            ...
            if ((request_type == 8'hA1) &&
              ...
              baud_shadow <= baud;
              stop_bits_shadow <= stop_bits;
              parity_shadow <= parity;
            end else if ((request_type == 8'h21) &&
        ...
        if (set_line_active && rx_active && rx_valid && (endpoint == 0)) begin
          case (data_index)
            0: baud_shadow[7:0] <= rx_data;
            1: baud_shadow[15:8] <= rx_data;
            2: baud_shadow[23:16] <= rx_data;
            3: baud_shadow[31:24] <= rx_data;
            4: stop_bits_shadow <= rx_data;
            5: parity_shadow <= rx_data;
            6: begin
              baud <= baud_shadow;
              stop_bits <= stop_bits_shadow;
              parity <= parity_shadow;
              data_bits <= rx_data;
              set_line_active <= 1'b0;
            end
