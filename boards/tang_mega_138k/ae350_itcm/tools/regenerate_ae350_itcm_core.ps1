...
$coreDir = Join-Path $projectRoot 'project\src\riscv_ae350_soc_itcm'
$configDir = Join-Path $coreDir 'config'
$firmwareDir = Join-Path $projectRoot 'firmware\itcm_initial'
$tempDir = Join-Path $coreDir 'temp\manual_138c_itcm'
...
foreach ($lane in 0..3) {
    $laneFile = Join-Path $firmwareDir "itcm$lane"
    if (-not (Test-Path -LiteralPath $laneFile -PathType Leaf)) {
        throw "ITCM-Initialisierungsdatei fehlt: $laneFile"
    }
}
...
function GowinPath([string]$Path) {
    return ([System.IO.Path]::GetFullPath($Path) -replace '\\', '/')
}

$firmwarePath = GowinPath $firmwareDir
$itcmConfig = @(
    '`define ITCM_SIZE 14'
    ('`define ITCM_INIT_PATH "{0}"' -f $firmwarePath)
)
$itcmConfig | Set-Content -LiteralPath (Join-Path $tempDir 'gw_itcm_config.v') -Encoding ascii

$data = GowinPath $ipData
$tmp = GowinPath $tempDir
$sources = @(
    'riscv_ae350_top.v',
    'riscv_ae350_soc.v',
    'riscv_ae350_ddr3.v',
...
$fileList
    ...
    <OptionList>
        ...
        <Option type="output_file" value="riscv_ae350_soc_itcm.vg"/>
        <Option type="output_template" value="riscv_ae350_soc_itcm_tmp.v"/>
...
$projectFile = Join-Path $tempDir 'riscv_ae350_soc_itcm.prj'
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "AE350-ITCM-IP-Synthese fehlgeschlagen (Exitcode $LASTEXITCODE)."
    }
} finally {
    Pop-Location
}

$generated = Join-Path $tempDir 'riscv_ae350_soc_itcm.vg'
$destination = Join-Path $coreDir 'riscv_ae350_soc_itcm.v'
Copy-Item -LiteralPath $generated -Destination $destination -Force
Write-Host "AE350-ITCM-Netzliste fuer GW5AST-138C erzeugt: $destination"
