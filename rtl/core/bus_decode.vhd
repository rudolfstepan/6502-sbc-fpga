...
begin
  ...
  begin
    ...
    -- 0x88B0-0x88BF: Math coprocessor (signed 32x32 fixed-point multiply)
      ...
      sel <= DEV_MATH;
    -- 0x88C0-0x88CF: Raw SD2 sector read/write controller
    elsif in_range(addr, ADDR_SDRAW_BASE, ADDR_SDRAW_LAST) then
      sel <= DEV_SDRAW;
    ...
    -- 0x9000-0x900F: VIC control and status registers (16 bytes)
      ...
      sel <= DEV_VIC_REG;
    -- 0xD000-0xD03F: VIC-II register block (border/background + $D011/$D012
    -- raster read-back, C64-compatible).
