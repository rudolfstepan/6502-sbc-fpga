...
architecture rtl of sbc_minimal_top is
  ...
  signal vic_mode_reg   : data_t := x"00";  -- $9000: bitmap, RGB332, framebuffer bank
  signal vic_text_attr  : data_t := x"00";  -- $9005: text attributes (bit1 = 80 columns)
  signal vic_fetch_bitmap : std_logic;

  -- Banked bitmap RAM (16 KB through the $6000-$7FFF CPU window)
...
begin
  ...
  cpu_rdy    <= not vic_stealing and not bitmap_wr_pending;
