...
begin
  ...
  cpu_rdy    <= sdram_rdy and not vic_stealing and not vram_wr_pending and not bitmap_wr_pending and not monitor_hold;
  ...
  -- a CPU bitmap write that collides with a VIC steal is held in bitmap_wr_*
  -- and committed on the next non-steal cycle. The CPU is kept stalled via
