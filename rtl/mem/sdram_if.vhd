...
--  * Does NOT start any access until ctrl_idle='1' (SDRAM init complete).
...
architecture rtl of sdram_if is
  ...
  type state_t is (S_IDLE, S_LOCKED, S_RD_REQ, S_RD_WAIT, S_WR_REQ, S_WR_WAIT);
    ...
    if rising_edge(clk) then
      if reset_n = '0' then
        ...
        case state is
          ...
          -- ---- Idle: accept new accesses only when SDRAM controller ready ----
          when S_IDLE =>
            wr_burst_req <= '0';
            rd_burst_req <= '0';
            rdy          <= '1';
