...
| `math_copro_test.prg` | Native C64 math-coprocessor smoke test for the `$DF00-$DF0F` I/O2 register window. It checks signed fixed-point multiplies in 8.24 and Q12 format. Build with `make c64-math-copro-test-prg`. |
| `mandelbrot_copro_c64.prg` | Native C64 Mandelbrot demo using the `$DF00-$DF0F` math coprocessor window. It renders a 160x200 VIC-II multicolour bitmap at `$2000`; reset after viewing. Build with `make c64-mandelbrot-copro-prg`. |
