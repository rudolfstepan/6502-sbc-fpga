...
| `mandelbrot_copro_c64.prg` | Native C64 Mandelbrot demo using software 8.24 fixed-point multiply, no coprocessor required. It renders a 160x200 VIC-II multicolour bitmap at `$2000`; reset after viewing. Build with `make c64-mandelbrot-prg` or the legacy alias `make c64-mandelbrot-copro-prg`. |
| `ich_image.prg` | Native C64 hires bitmap image viewer built from the existing `sw/ich_image_c16.bin` payload. Build with `make c64-ich-image-prg`, upload with `tools/c64_uart_prg_loader.py`, then `RUN`. |
