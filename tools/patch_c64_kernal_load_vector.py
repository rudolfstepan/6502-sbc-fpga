...
RAM at $C000 after upload.  A direct `JMP $C006` would be unsafe before the
...
tiny guard stub in unused KERNAL space:
    ...
    $FFD5: JMP $ECB9
    $ECB9: PHA
           if $C000 == $2C then PLA : JMP $C006 else PLA : JMP $F49E

A holds the KERNAL LOAD/VERIFY flag at $FFD5, so the stub must preserve it
around the LDA $C000 signature check; clobbering A turns every LOAD into a
...
HOOK_BASE = 0xC000
HOOK_ENTRY = HOOK_BASE + 0x0006
HOOK_GUARD_ADDR = HOOK_BASE
HOOK_GUARD_VALUE = 0x2C  # BIT abs signature at the start of c64_sd_fastload_hook.s
...
STUB_BYTES = bytes(
    (
        ...
        0xC9, HOOK_GUARD_VALUE,                              # CMP hook signature byte
        0xD0, 0x04,                                          # BNE stock
        0x68,                                                # PLA (restore A)
