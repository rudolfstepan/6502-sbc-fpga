...
| $D000-$EFFF | 8 KB | I/O / reserved | VIC-II regs $D000-$D03F (border/background + $D011/$D012 raster read-back); SID at $D400-$D41C (incl. OSC3/ENV3 reads); C64 host UART at $DE00/$DE01; `$DF00-$DFFF` currently returns open-bus `$FF` in the stable C64 bitstream; not shadow ROM |
